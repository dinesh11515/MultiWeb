...
const SelectChain = ({ setPage, page, formData, setFormData }) => {
  ...
  const connectWalletHandler = async () => {
    try {
      if (chain.chainName === "Polygon Mumbai Testnet") {
        setDefaultChain(polygonMumbai);
      }
      if (chain.chainName === "Bsc Testnet") {
        setDefaultChain(bscTestnet);
      }
      if (chain.chainName === "Avalanche Fuji Testnet") {
        setDefaultChain(avalancheFuji);
      }
      if (chain.chainName === "Base Goerli Testnet") {
        setDefaultChain(baseGoerli);
      }
      if (chain.chainName === "Moonbase Alpha Testnet") {
        setDefaultChain(moonbaseAlpha);
      }
      if (chain.chainName === "Celo Alfajores Testnet") {
